...
<script>
// stop propagation of keydown event is to make shortcuts like d (switch dark mode), f (switch fullscreen) disabled

// source: https://raw.githubusercontent.com/SyedWasiHaider/vue-highlightable-input/master/src/HighlightableInput.vue
  ...
  methods: {
    ...
    processHighlights() {
      ...
      if (startingPosition < this.internalValue.length)
        result += this.safe_tags_replace(
          this.internalValue.substring(startingPosition, this.internalValue.length),
        )
    ...
    normalizedHighlights() {
      ...
      if (
        ...
        return nonRegExpHighlights
          .map((h) => {
            if (h.text != undefined || typeof h == 'string') {
              return {
                text: h.text || h,
    ...
    getRegexIndices(regex, str) {
      ...
      while ((match = regex.exec(str)) != null) {
        if (indices.length > 0 && match.index === indices[indices.length - 1].start) {
          break
        }
